...
- Authentication is optional - controlled by --auth flag
  ...
  - `authMiddleware` answers `POST /upload` with a JSON 401 rather than the 303 page routes get, since `fetch` follows a redirect and hands the client login HTML; the method matters because `GET /upload` is a real file path through the download catch-all, and it keeps the 303 wherever the read group still runs the middleware
